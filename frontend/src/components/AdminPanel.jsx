                ...
                {g.items.map(s => (
                  <div key={s.id} style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 10, alignItems: 'flex-start' }}>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      {s.image && <img src={s.image} alt="evidencia" style={{ height: 40, borderRadius: 6, cursor: 'zoom-in' }} onClick={() => setPreview({ src: s.image, kind: 'image' })} />}
                      {s.video && <video src={s.video} muted style={{ height: 40, borderRadius: 6, cursor: 'pointer', background: '#000' }} onClick={() => setPreview({ src: s.video, kind: 'video' })} />}
                    </div>
