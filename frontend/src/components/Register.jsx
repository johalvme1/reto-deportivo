...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(data.message || 'Registro exitoso. Espera la aprobación del administrador.');
    } catch (err) {
        ...
        {success && (
          <div className="alert alert-success" style={{ textAlign: 'center' }}>
            ...
            <div style={{ marginTop: 10 }}>
              <Link to="/login" className="btn btn-primary btn-sm">Ir a iniciar sesión</Link>
            </div>
          </div>
        )}
        {!success && (
